...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, in_channels=3):
        ...
        self.hybrid = None
        # img_size = _pair(in_feat_size)
        in_feat_size = config.input_dim
        token_num = config.token_num
        self.patch_embeddings = nn.Linear(in_feat_size,in_feat_size)
        self.position_embeddings = nn.Parameter(torch.zeros(1, token_num+1, config.hidden_size))
    ...
    def forward(self, x):
        ...
        x = x.flatten(2) # (batch, token_num, w2v)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=4, zero_head=False, vis=False):
        ...
        self.head = Linear(config.hidden_size, 21843)
...
class AutoEncoder64(nn.Module):
    def __init__(self, zdim):
        ...
        self.encoder = nn.Sequential(
                    ...
                    nn.BatchNorm2d(128),
                    nn.LeakyReLU(inplace=True),
                    nn.Conv2d(in_channels=128, out_channels=zdim, kernel_size=3, stride=2, bias=False),
                    nn.BatchNorm2d(zdim),
                    nn.LeakyReLU(inplace=True),
        )
        ...
        self.upsample = nn.Sequential(
            torch.nn.Upsample(scale_factor=3, mode='nearest'),
            nn.LeakyReLU(inplace=True),
            torch.nn.Linear(in_features=3, out_features=1, bias=True),
            nn.LeakyReLU(inplace=True))
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(in_channels=zdim, out_channels=128, kernel_size=3, stride=2, bias=False),
            nn.BatchNorm2d(128),
            nn.LeakyReLU(inplace=True),
            ...
            nn.BatchNorm2d(32),
            nn.LeakyReLU(inplace=True),
            nn.ConvTranspose2d(in_channels=32, out_channels=3, kernel_size=3, stride=2, bias=False, output_padding=1),
            nn.Sigmoid()
        )
    
    def encode(self, x):
        z_down = self.encoder(x)
        z = self.downsample(z_down)
        return z

    def decode(self, z):
        z_up = self.upsample(z)
        z_up = torch.transpose(z_up, 2, 3)
        y = self.decoder(z_up)
        return y
...
class AutoEncoder(nn.Module):
    def __init__(self, zdim):
        ...
        self.encoder = nn.Sequential(
                    ...
                    nn.BatchNorm2d(256),
                    nn.LeakyReLU(inplace=True),
                    nn.Conv2d(in_channels=256, out_channels=zdim, kernel_size=3, stride=2, bias=False),
                    nn.BatchNorm2d(zdim),
                    nn.LeakyReLU(inplace=True),
        )
        self.downsample = nn.Sequential(
        ...
        self.upsample = nn.Sequential(
            ...
            nn.LeakyReLU(inplace=True))
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(in_channels=zdim, out_channels=256, kernel_size=3, stride=2, bias=False, dilation=(1,1)),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(inplace=True),
...
class ResNet50(nn.Module):
    def __init__(self,class_num,feat_dim,input_channel=3):
        ...
        self.net.fc = nn.Linear(num_ftrs, feat_dim)
        self.fc = nn.Sequential(nn.ReLU(),nn.Dropout(p=0.5),nn.Linear(feat_dim, class_num))
    def forward(self,x):
        feat = self.net(x)
        y = self.fc(feat)
        return y,feat
...
class GateNet(nn.Module):
    def __init__(self,input_dim):
        ...
        self.weight = nn.Sequential(nn.Linear(input_dim, 1),nn.Sigmoid())
    def forward(self,x):
        feat = self.embed(x)
...
CONFIGS = {
    ...
    'word_level_config_only_design':configs.word_level_config_only_design(),
