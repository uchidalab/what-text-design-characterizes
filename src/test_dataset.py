...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, semantic, font_style, char_color, bk_color, height, coord, label_list, all_word_list, transform=None):
    ...
    def __getitem__(self, idx):
        vec = {}
        vec["semantic"] = self.semantic[idx]
        vec["font_style"] = self.font_style[idx]
        vec["char_color"] = self.char_color[idx]
        vec["bk_color"] = self.bk_color[idx]
        vec["height"] = self.height[idx]
        vec["coord"] = self.coord[idx]
...
def get_test_dataloader(batch_size,
    ...
    # Load test data only
    df = pd.read_csv(f"{amazon_csv_path}/test.csv")
    df = df[df['split'] == 'test'].reset_index(drop=True)
    df = df[df['hight'] > 14]
    df = df[df['width'] > 14]
    
    df_org = pd.read_csv(f'{book_listing_csv_path}/book30-listing-test.csv',
                        encoding='cp932',
                        names=("Amazon ID (ASIN)", "Filename", "Image URL", "Title", "Author", "Category ID", "Category"))
    ...
    zero_vec = torch.zeros((1, 300)).to(device)
    
    if debug:
        book_cover_list = book_cover_list[0:5]
    
    # Process each book cover
    for i in tqdm(range(len(book_cover_list)), desc="Processing test data"):
        semantic_list = []
        font_style_list = []
        char_color_list = []
        bk_color_list = []
        ...
        for j in range(token_num):
            if len(df_tmp) > j:
                ...
                semantic_vec = semantic_vec.reshape((1, 300))
                semantic_list.append(semantic_vec)
                ...
                try:
                    ...
                    img = torch.zeros((1, 3, 64, 128)).to(device)
                
                _, font_vec = model_font(img.to(device))
                font_style_list.append(font_vec)
                
                # Get color features
                ch_color, bk_color = get_color(img_path)
                char_color_list.append(ch_color.reshape((1, 300)).to(device))
                bk_color_list.append(bk_color.reshape((1, 300)).to(device))
                ...
                # Padding with zeros
                semantic_list.append(zero_vec)
                font_style_list.append(zero_vec)
                char_color_list.append(zero_vec)
                bk_color_list.append(zero_vec)
                height_list.append(torch.tensor([0]))
                coord_list.append(torch.tensor([0, 0]))
                title_words += '* '
        
        # Stack and append to main lists
        semantic_list = torch.stack(semantic_list)
        all_semantic_list.append(semantic_list)
        
        font_style_list = torch.stack(font_style_list)
        all_font_style_list.append(font_style_list)
        
        char_color_list = torch.stack(char_color_list)
        all_char_color_list.append(char_color_list)
        
        bk_color_list = torch.stack(bk_color_list)
        all_bk_color_list.append(bk_color_list)
    ...
    dataset = TestDataset(all_semantic_list, all_font_style_list, all_char_color_list, all_bk_color_list,
                         all_height_list, all_coord_list, label_list, all_title_list, transform=trans1)
