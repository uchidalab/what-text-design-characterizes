...
def get_color_v2(img_path): 
    ...
    value, count = np.unique((img_char_color.reshape(-1, img_char_color.shape[1])), axis=0, return_counts=True)
    # Get top 100 most frequent RGB colors
    idx = np.argsort(count)[::-1][0]
    char_color_tensor = value[idx]
    img_back_st = np.dstack((img_back,img_back))
    img_back_st = np.dstack((img_back_st,img_back))
    img_back_color = img_Lab * img_back_st
    img_back_color = img_back_color[np.all(img_back_color>0, axis=2)]
...
def get_dataloader(batch_size,
    ...
    all_height_list = []
    all_coord_list = []
    zero_vec = torch.zeros((1,300)).to(device) # For semantic, font, ch_color, bk_color
    if debug == True:
        if data_type == 'train':
    ...
    dataset = Mydatasets(all_semantic_list, all_font_style_list, all_char_color_list,all_bk_color_list,
                         all_height_list,coord_list,label_list,all_title_list,transform=trans1)
